...
const createUserProfile = async (req, res, next) => {
  ...
  try {
    const data = await User_Profile.create({
      name: name,
      profile_image_url: process.env.HOSTNAME + `/public/profile-image/${profilePicture.uuid}.${profilePicture.type}`,
      quotes: quotes ?? '-',
      facebook: facebook ?? '-',
...
const updateUserProfile = async (req, res, next) => {
  ...
  try {
    ...
    let imageRelativeLocation = imageFixLocation.replace(process.env.HOSTNAME + '/public/profile-image/', '')
    ...
    if (profilePicture) {
      Object.assign(updateData, { profile_image_url: process.env.HOSTNAME + `/public/profile-image/${profilePicture.uuid}.${profilePicture.type}` });
    }
